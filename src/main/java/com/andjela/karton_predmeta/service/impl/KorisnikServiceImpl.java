...
public class KorisnikServiceImpl implements KorisnikService {
    ...
    public String registruj(String password, String ime, String prezime, String brojIndeksa, Long programId) {
        if (password == null || password.trim().isEmpty())
            throw new ValidationException("Lozinka je obavezna.");
        if (ime == null || ime.trim().isEmpty())
            throw new ValidationException("Ime je obavezno.");
        if (prezime == null || prezime.trim().isEmpty())
            throw new ValidationException("Prezime je obavezno.");
        if (brojIndeksa == null || brojIndeksa.trim().isEmpty())
            throw new ValidationException("Broj indeksa je obavezan.");
        if (programId == null)
            throw new ValidationException("Smer je obavezan.");
        ...
        if (korisnikRepository.findByUsername(username).isPresent())
            throw new ValidationException("Korisnik sa ovim podacima već postoji (korisničko ime " + username + " je zauzeto).");

        Uloga studentUloga = ulogaRepository.findByNaziv("STUDENT")
                .orElseThrow(() -> new NotFoundException("Uloga STUDENT ne postoji."));

        StudijskiProgram program = studijskiProgramRepository.findById(programId)
                .orElseThrow(() -> new NotFoundException("Smer ne postoji."));
